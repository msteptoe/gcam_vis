# Only for testing exclue otherwise
...
firstScenario <- TRUE
customQuery <- TRUE
queryFile <- "D:/msteptoe/Documents/R/win-library/3.4/rgcam/ModelInterface/sample-queries.xml"
...
# directories <- c("QueryVectors/", "ClusterAssignments/", "PCAs/", "LineCharts/", "Maps/")
...
for(query in queryList){
  ...
  for(col in cols){
    ...
    if(col != 'value' & col != 'region'){
      ...
      for(key in keys[[col]]){
        if(firstScenario){
          toCreate <- paste0(dataDir, "Query/", query, "/", col, "/", key, "/")
          dir.create(toCreate, recursive=TRUE)
        }
        valueVector <- queryResults[queryResults[[col]] == key,]
        jsonFile <- paste0(dataDir, "Query/", query, "/", col, "/", key, "/", scenarioName, ".json")
  ...
  if(length(cols) <= 2){
    jsonFile <- paste0(dataDir, "Query/", query, "/", scenarioName, ".json")
