...
function addParAxis() {
	...
	if (!state.parCoor.axisKeys[query]) {
		if (variable) {
			state.parCoor.axisKeys[query] = {};
			state.parCoor.axisKeys[query][variable] = [value]
		}
		else {
			state.parCoor.axisKeys[query] = ['value'];
		}
	}
	else if (state.parCoor.axisKeys[query]) {
		if (variable) {
			if (state.parCoor.axisKeys[query][variable]) {
				if (state.parCoor.axisKeys[query][variable].indexOf(value) == -1){
					state.parCoor.axisKeys[query][variable].push(value);
				}
				else{
					return;
				}
			...
			console.log('Should not be in this case of addParAxis()');
		}
	}

	socket.getParCoorAxis(state.parCoor.files, undefined, query, variable, value);
}
