...
var figue = function () {
	...
	function agglomerateDone(){
		console.log('agglomerateDone!');
	}

	function agglomerateMain (mainObj){
		var c1, c2, c1Cluster, c2Cluster, i, j, p, newCentroid;

		// Main loop
		if (mainObj.p < mainObj.N-1 ) {
			// find the closest pair of clusters
			c1 = 0 ;
			for (i = 0 ; i < mainObj.N ; i++) {
				if (mainObj.distMatrix[i][mainObj.dMin[i]] < mainObj.distMatrix[c1][mainObj.dMin[c1]])
					c1 = i ;
			}
			c2 = mainObj.dMin[c1] ;
	
			// create node to store cluster info 
			c1Cluster = mainObj.clusters[c1][0] ;
			c2Cluster = mainObj.clusters[c2][0] ;

			newCentroid = calculateCentroid ( c1Cluster.size , c1Cluster.centroid , c2Cluster.size , c2Cluster.centroid ) ;
			newCluster = new Node (-1, c1Cluster, c2Cluster , mainObj.distMatrix[c1][c2] , newCentroid) ;
			mainObj.clusters[c1].splice(0,0, newCluster) ;
			mainObj.cSize[c1] += mainObj.cSize[c2] ;
		
			// overwrite row c1 with respect to the linkage type
			for (j = 0 ; j < mainObj.N ; j++) {
				if (mainObj.linkage == figue.SINGLE_LINKAGE) {
					if (mainObj.distMatrix[c1][j] > mainObj.distMatrix[c2][j])
						mainObj.distMatrix[j][c1] = mainObj.distMatrix[c1][j] = mainObj.distMatrix[c2][j] ;
				} else if (mainObj.linkage == figue.COMPLETE_LINKAGE) {
					if (mainObj.distMatrix[c1][j] < mainObj.distMatrix[c2][j])
						mainObj.distMatrix[j][c1] = mainObj.distMatrix[c1][j] = mainObj.distMatrix[c2][j] ;
				} else if (mainObj.linkage == figue.AVERAGE_LINKAGE) {
					var avg = ( mainObj.cSize[c1] * mainObj.distMatrix[c1][j] + mainObj.cSize[c2] * mainObj.distMatrix[c2][j])  / (mainObj.cSize[c1] + mainObj.cSize[j]) 
					mainObj.distMatrix[j][c1] = mainObj.distMatrix[c1][j] = avg ;
				}
			}
			mainObj.distMatrix[c1][c1] = Infinity ;
		
			// infinity ­out old row c2 and column c2
			for (i = 0; i < mainObj.N ; i++)
				mainObj.distMatrix[i][c2] = mainObj.distMatrix[c2][i] = Infinity ;
	
			// update dmin and replace ones that previous pointed to c2 to point to c1
			for (j = 0; j < mainObj.N ; j++) {
				if (mainObj.dMin[j] == c2)
					mainObj.dMin[j] = c1;
				if (mainObj.distMatrix[c1][j] < mainObj.distMatrix[c1][mainObj.dMin[c1]]) 
					mainObj.dMin[c1] = j;
			}
	
			// keep track of the last added cluster
			mainObj.root = newCluster ;
			mainObj.p++;
			setTimeout(agglomerateMain.bind(null, mainObj), 50);
		}
		else{
			figue.agglomerateDone(mainObj.root);
			return;
		}
	}

	function agglomerateDelayed (labels, vectors, distance, linkage) {
		var N = vectors.length ;
		var dMin = new Array(N) ;
		var cSize = new Array(N) ;
		var matrixObj = new figue.Matrix(N,N);
		...
		var mainObj = {p: 0, N: N, dMin: dMin, cSize: cSize, distMatrix: distMatrix, clusters: clusters, linkage: linkage, root: null};
		agglomerateMain(mainObj);
	}
	...
	return { 
		...
		agglomerateDelayed: agglomerateDelayed,
		agglomerateDone: agglomerateDone,
		kmeans: kmeans,
		fcmeans: fcmeans
	}
}() ;
