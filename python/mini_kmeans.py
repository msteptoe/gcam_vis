...
def mainFunc(): 
    ...
    for n_clusters in range(3, 9):
        if n_clusters <= len(files):
            # kmeans = KMeans(n_clusters=n_clusters, random_state=0, n_jobs=3).fit(X)
            ...
            dump(kmeans.cluster_centers_.tolist(), open(jobPath + os.path.sep + 'ClusterCenters' + os.path.sep + query + os.path.sep + "k" + str(n_clusters) + ".json", "w"))
